...
missing_required_callback_fails(_Config) ->
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            assert_error(
                {erlando_validation, {missing_callbacks, Instance, Typeclass, [{required, 1}]}},
...
optional_callback_is_not_required(_Config) ->
    ...
    with_beams(
        [
            ...
            {Instance, instance_forms(Instance, optional_type, [Typeclass], [{required, 1}])}
        ],
        fun(Beamfiles) ->
            State = add_fixture_modules(Typeclass, [Instance], Beamfiles),
            {ok, typeclass, _} = rebar3_erlando_compile:compile(State),
...
undeclared_gen_fun_capability_fails(_Config) ->
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            ...
            Attributes = [
                {gen_fun, [#{behaviours => [undeclared_capability]}]}
                | Attributes0
            ],
...
retained_gen_fun_metadata_is_validated(_Config) ->
    ...
    Forms =
        [
            ...
            {attribute, 1, gen_fun_meta, {1, #{behaviours => [undeclared_capability]}}},
...
unknown_gen_fun_metadata_version_fails(_Config) ->
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            assert_error(
                {erlando_validation, {unsupported_gen_fun_metadata_version, Instance, 99}},
                fun() -> add_fixture_modules(Typeclass, [Instance], Beamfiles) end
            )
        end
    ).
...
metadata_defines_exact_mappings(_Config) ->
    ...
    Metadata =
        {1, #{
            ...
            instances =>
                [
                    #{
                        type => first_metadata_type,
                        typeclass => FirstClass,
                        implementation => local,
                        adapter => manual
                    },
                    #{
                        type => second_metadata_type,
                        typeclass => SecondClass,
                        implementation => local,
                        adapter => manual
                    }
                ]
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            ...
            assert_exit(
                {unregisted_module, {first_metadata_type, SecondClass}},
                fun() -> typeclass:module(first_metadata_type, SecondClass) end
            ),
            assert_exit(
                {unregisted_module, {second_metadata_type, FirstClass}},
...
unknown_metadata_version_fails(_Config) ->
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            assert_error(
                {erlando_validation, {unsupported_instance_metadata_version, Instance, 99}},
                fun() -> add_fixture_modules(Typeclass, [Instance], Beamfiles) end
            )
        end
    ).
...
missing_dispatch_callback_fails(_Config) ->
    ...
    Forms = dispatch_instance_forms(
        Instance,
        dispatch_coverage_type,
        Typeclass,
        Metadata,
        [{run, 1}]
    ),
    with_beams(
        ...
        fun(Beamfiles) ->
            assert_error(
                {erlando_validation,
                    {missing_dispatch_callback, Instance, dispatch_coverage_type, Typeclass,
                        {run, 1}}},
                fun() -> add_fixture_modules(Typeclass, [Instance], Beamfiles) end
            )
        end
...
missing_dispatch_adapter_fails(_Config) ->
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            assert_error(
                {erlando_validation,
                    {missing_dispatch_adapter, Instance, dispatch_adapter_type, Typeclass, {run, 1},
                        {missing_adapter, 1}}},
                fun() -> add_fixture_modules(Typeclass, [Instance], Beamfiles) end
            )
        end
...
function_type_patterns_classify_fun_values(_Config) ->
    ...
    FunType = {type, 1, 'fun', [{type, 1, product, [{type, 1, any, []}]}, {type, 1, any, []}]},
    RefForms =
        [
    ...
    with_beams(
        ...
        fun(Beamfiles) ->
            Types = [
                {BareInstance, beam_attributes(BareInstance, Beamfiles)},
...
function_form({Name, Arity}) ->
    Arguments = [
        {var, 1, list_to_atom("Arg" ++ integer_to_list(N))}
     || N <- lists:seq(1, Arity)
    ],
...
with_beams(ModuleForms, Test) ->
    ...
    after
        ...
        lists:foreach(
            fun({_Module, Beamfile}) -> ok = file:delete(Beamfile) end,
